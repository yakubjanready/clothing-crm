...
class FinancePayment(Base, UUIDPrimaryKeyMixin, TimestampMixin):
    ...
    __table_args__ = (CheckConstraint("amount > 0", name="ck_fp_amount_positive"),)

    number: Mapped[str] = mapped_column(String(32), unique=True, nullable=False, index=True)
    direction: Mapped[PaymentDirection] = mapped_column(String(16), nullable=False, index=True)
    category: Mapped[FinanceCategory] = mapped_column(String(32), nullable=False, index=True)
