...
@pytest.fixture
async def catalog_setup(client: AsyncClient, admin_headers: dict[str, str]) -> tuple[str, str, str]:
    """Kategoriya + brend + mahsulot + 1 variant — keyingi testlarda ishlatish uchun."""
    cat = (
        await client.post("/api/v1/categories", headers=admin_headers, json={"name": "Maykalar"})
    ).json()
    brand = (
        await client.post("/api/v1/brands", headers=admin_headers, json={"name": "Nike"})
    ).json()
    prod = (
        await client.post(
            "/api/v1/products",
...
@pytest.fixture
async def two_warehouses(client: AsyncClient, admin_headers: dict[str, str]) -> tuple[str, str]:
    a = (
        await client.post(
            "/api/v1/warehouses",
            headers=admin_headers,
...
async def test_warehouse_filter_by_type(client: AsyncClient, admin_headers: dict[str, str]) -> None:
    ...
    r = await client.get("/api/v1/warehouses", headers=admin_headers, params={"type": "branch"})
...
async def _reserve(client, headers, variant, warehouse, qty):
    return await client.post(
        "/api/v1/stock/movements/reserve",
        headers=headers,
...
async def _release(client, headers, variant, warehouse, qty):
    return await client.post(
        "/api/v1/stock/movements/release",
        headers=headers,
        json={"variant_id": variant, "warehouse_id": warehouse, "quantity": qty},
    )
...
async def test_receive_creates_stock_and_movement(
    ...
    catalog_setup: tuple[str, str, str],
    two_warehouses: tuple[str, str],
...
async def test_issue_decrements_and_logs(
    ...
    listed = await client.get("/api/v1/stock", headers=admin_headers, params={"warehouse_id": a})
...
async def test_transfer_atomic_between_warehouses(
    ...
    stocks = (
        (await test_db.execute(select(Stock).where(Stock.variant_id == uuid.UUID(variant_id))))
        .scalars()
        .all()
    )
...
async def test_inventory_session_count_and_finalize(
    ...
    r = await client.post(
        f"/api/v1/inventory/{inv['id']}/items",
        headers=admin_headers,
    ...
    fin = await client.post(f"/api/v1/inventory/{inv['id']}/finalize", headers=admin_headers)
...
async def test_inventory_cannot_count_after_finalize(
    ...
    a, _ = two_warehouses
    inv = (
        await client.post("/api/v1/inventory", headers=admin_headers, json={"warehouse_id": a})
    ).json()
    await client.post(f"/api/v1/inventory/{inv['id']}/finalize", headers=admin_headers)
    r = await client.post(
        f"/api/v1/inventory/{inv['id']}/items",
        headers=admin_headers,
...
async def test_movements_history_filter_by_type(
    ...
    r = await client.get("/api/v1/stock/movements", headers=admin_headers, params={"type": "in"})
