...
async def test_account_name_unique_409(client: AsyncClient, admin_headers: dict[str, str]) -> None:
    await _create_account(client, admin_headers, "Dup")
    r = await client.post("/api/v1/accounts", headers=admin_headers, json={"name": "Dup"})
    assert r.status_code == 409


async def test_account_filter_by_type(client: AsyncClient, admin_headers: dict[str, str]) -> None:
    ...
    r = await client.get("/api/v1/accounts", headers=admin_headers, params={"type": "bank"})
...
async def test_payments_filter_by_direction_and_account(
    ...
    for d, amt in [("income", "1000"), ("income", "500"), ("expense", "200")]:
        await client.post(
            "/api/v1/payments",
            headers=admin_headers,
            json={"direction": d, "category": "other", "account_id": a["id"], "amount": amt},
        )
