...
async def test_inn_unique_409(client: AsyncClient, admin_headers: dict[str, str]) -> None:
    await client.post(
        "/api/v1/customers",
        headers=admin_headers,
...
async def test_customer_filter_by_segment_and_search(
    ...
    vips = await client.get("/api/v1/customers", headers=admin_headers, params={"segment": "vip"})
    assert vips.json()["total"] == 1

    savdo = await client.get("/api/v1/customers", headers=admin_headers, params={"search": "savdo"})
...
async def test_sales_user_can_read_but_not_delete(
    ...
    cust = (
        await client.post("/api/v1/customers", headers=admin_headers, json={"name": "Z"})
    ).json()
    # sales can read
    r = await client.get(f"/api/v1/customers/{cust['id']}", headers=sales_headers)
    assert r.status_code == 200
    # sales cannot delete
    d = await client.delete(f"/api/v1/customers/{cust['id']}", headers=sales_headers)
...
async def test_add_and_list_contacts(
    ...
    listed = await client.get(f"/api/v1/customers/{customer_id}/contacts", headers=admin_headers)
    items = listed.json()
    assert len(items) == 2
    # is_primary birinchi
...
async def test_setting_new_primary_unsets_old(
    ...
    listed = (
        await client.get(f"/api/v1/customers/{customer_id}/contacts", headers=admin_headers)
    ).json()
...
async def test_balance_aggregates_correctly(
    ...
    r = await client.get(f"/api/v1/customers/{customer_id}/balance", headers=admin_headers)
