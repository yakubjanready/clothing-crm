...
@pytest.fixture
async def supplier_id(client: AsyncClient, admin_headers: dict[str, str]) -> str:
    s = (
        await client.post(
            "/api/v1/suppliers",
            headers=admin_headers,
...
async def test_supplier_name_unique_409(client: AsyncClient, admin_headers: dict[str, str]) -> None:
    await client.post("/api/v1/suppliers", headers=admin_headers, json={"name": "Dup"})
    dup = await client.post("/api/v1/suppliers", headers=admin_headers, json={"name": "Dup"})
...
async def test_supplier_filter_min_rating(
    ...
    r = await client.get("/api/v1/suppliers", headers=admin_headers, params={"min_rating": 3})
...
async def test_sales_user_cannot_create_supplier(
    ...
    r = await client.post("/api/v1/suppliers", headers=sales_headers, json={"name": "X"})
...
async def test_receive_creates_stock_and_updates_cost_and_debt(
    ...
    _, variant_id = catalog_setup
    po = await _create_po(client, admin_headers, supplier_id, warehouse_id, variant_id, 50, "7500")
    r = await client.post(f"/api/v1/purchase-orders/{po['id']}/receive", headers=admin_headers)
    ...
    movs = (
        (await test_db.execute(select(StockMovement).where(StockMovement.type == MovementType.IN)))
        .scalars()
        .all()
    )
...
async def test_sales_user_cannot_receive(
    ...
    _, variant_id = catalog_setup
    po = await _create_po(client, admin_headers, supplier_id, warehouse_id, variant_id)
    r = await client.post(f"/api/v1/purchase-orders/{po['id']}/receive", headers=sales_headers)
    assert r.status_code == 403
...
async def test_pay_full_marks_paid_and_clears_debt(
    ...
    # debt = 50000
    r = await client.post(
        f"/api/v1/purchase-orders/{po['id']}/pay",
        headers=admin_headers,
    ...
    # PO status PAID
    list_resp = await client.get(f"/api/v1/purchase-orders/{po['id']}", headers=admin_headers)
...
async def test_pay_partial_keeps_received_status(
    ...
    detail = await client.get(f"/api/v1/purchase-orders/{po['id']}", headers=admin_headers)
...
async def test_cancel_draft_only(
    ...
    r2 = await client.post(f"/api/v1/purchase-orders/{po['id']}/receive", headers=admin_headers)
...
async def test_supplier_balance_aggregates(
    ...
    await client.post(
        f"/api/v1/purchase-orders/{p1['id']}/pay",
        headers=admin_headers,
        json={"amount": "3000"},
    )
    ...
    bal = (
        await client.get(f"/api/v1/suppliers/{supplier_id}/balance", headers=admin_headers)
    ).json()
