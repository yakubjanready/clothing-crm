...
async def cat_brand(
    ...
    cat = (await client.post(
        "/api/v1/categories", headers=admin_headers,
        json={"name": "Maykalar"},
    )).json()
    brand = (await client.post(
        "/api/v1/brands", headers=admin_headers,
...
async def test_category_parent_child_tree(
    ...
    parent = (await client.post(
        "/api/v1/categories", headers=admin_headers, json={"name": "Erkaklar"}
    )).json()
...
async def test_category_slug_unique_409(
    client: AsyncClient, admin_headers: dict[str, str]
) -> None:
    await client.post(
        "/api/v1/categories", headers=admin_headers,
...
async def test_brand_create_and_filter_by_country(
    ...
    r = await client.get(
        "/api/v1/brands", headers=admin_headers, params={"country": "USA"}
    )
...
async def test_product_filter_by_gender_brand_search(
    ...
    other_brand = (await client.post(
        "/api/v1/brands", headers=admin_headers, json={"name": "Puma"}
    )).json()
    ...
    for name, gender, b in [
        ...
        await client.post(
            ...
            json={"name": name, "gender": gender,
                  "category_id": cat_id, "brand_id": b},
        )

    men = await client.get(
        "/api/v1/products", headers=admin_headers, params={"gender": "men"}
    )
...
async def test_create_single_variant_with_auto_sku(
    ...
    r = await client.post(
        f"/api/v1/products/{product_id}/variants", headers=admin_headers,
        json={
            "size": "M", "color": "Qora",
            "wholesale_price": "120000", "retail_price": "180000",
        },
    )
    assert r.status_code == 201, r.text
    body = r.json()
...
async def test_variant_matrix_3_sizes_x_2_colors(
    client: AsyncClient, admin_headers: dict[str, str],
    product_id: str, test_db: AsyncSession
) -> None:
    r = await client.post(
    ...
    rows = (
        await test_db.execute(
            select(ProductVariant).where(
                ProductVariant.product_id == _uuid.UUID(product_id)
            )
        )
    ).scalars().all()
    assert len(rows) == 6
...
async def test_sales_user_cannot_create_product(
    ...
    cat = (await client.post(
        "/api/v1/categories", headers=admin_headers, json={"name": "Foo"}
    )).json()
    r = await client.post(
        "/api/v1/products", headers=sales_headers,
...
async def test_upload_image_jpeg_success(
    ...
    r = await client.post(
        "/api/v1/upload/image", headers=admin_headers, files=files
    )
    assert r.status_code == 201, r.text
    body = r.json()
...
async def test_upload_image_requires_product_write(
    ...
    r = await client.post(
        "/api/v1/upload/image", headers=sales_headers, files=files
    )
    assert r.status_code == 403
